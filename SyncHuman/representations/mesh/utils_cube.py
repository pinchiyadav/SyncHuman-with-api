...
def get_defomed_verts_fourtimes(v_pos : torch.Tensor, deform : torch.Tensor, res):
    return v_pos / res - 0.5 + 2*(1 - 1e-8) / (res) * torch.tanh(deform)

def get_defomed_verts_eightimes(v_pos : torch.Tensor, deform : torch.Tensor, res):
    return v_pos / res - 0.5 + 4*(1 - 1e-8) / (res) * torch.tanh(deform)
