...
def render_multiview(sample, resolution=512, nviews=30):
    r = 2
    fov = 40
    yaws, pitchs = [-np.pi,-np.pi*3/2 , 0,-np.pi/2, -np.pi], [0, 0, 0, 0,0]
    cams = [sphere_hammersley_sequence(i, nviews-5) for i in range(nviews-5)]
    yaws = yaws+[cam[0] for cam in cams]
    pitchs = pitchs+[cam[1] for cam in cams]
